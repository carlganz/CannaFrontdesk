...
patientInfoUI <- function(id) {
  ...
  tagList(div(class = "content",
              ...
              div(class = "col-xs-6 col-sm-6 col-md-6 col-lg-6",
                  div(class = "row",
                      div(class = "add-delete-btn-container",style = "width:100%;",
                          div(
                            tags$button(id = ns("add_queue"), "Add to Queue", class = "btn btn-info add-queue-btn action-button", 
                                        style = "width:30%;"),
                            tags$button(id = ns("let_in"), "Let in Store", class = "btn btn-info add-queue-btn action-button", 
                                        style = "width:30%;"),
                  ...
                  box(
                    tableTitle("Medical Info"),
                    DT::dataTableOutput(ns("recommendation"))#,
                    # shiny::uiOutput(ns("recommendation_image_out"))
                  ),
                  box(class = "images",
                    tableTitle("Images"),
                    div(class = "col-xs-12 col-sm-12 col-md-12 col-lg-12",
                        ...
                        div(class = "col-xs-6 col-sm-6 col-md-6 col-lg-6",
                            h3("Medical Card"),
                            uiOutput(ns("recommendation_image_out"))
                        )
                            )
                  ),
                  box(
...
patientInfo <- function(input, output, session, pool, patientId, bucket, queue, trigger_queue, reload, trigger_new, trigger_returning, proxy, parent_session, reload_patient) {
  ...
  observeEvent(input$edit_basic_info, {
    showModal(modalDialog(size= "l",
      ...
      tags$form(id = session$ns("basic_info_form"),
                div(class = "col-xs-6 col-sm-6 col-md-6 col-lg-6",
      input(session$ns("name"), placeholder = "First", label_width = 4, value = patient_info_returning()$firstName, input_width = 8),
      input(session$ns("name2"), placeholder = "Last", label_width = 4, value = patient_info_returning()$lastName, input_width = 8),
      input(session$ns("californiaID"), type = "tel", placeholder = "California ID", label = "CA ID", label_width = 4, maxlength = 8,
            `data-parsley-californiaid` = I(""), value = patient_info_returning()$californiaID, input_width = 8),
      input(session$ns("birthday"), placeholder = "DOB", label = "DOB", label_width = 4, `data-parsley-year` = I(""),
            `data-parsley-pattern` = "/^\\d{4}[\\/\\-](0?[1-9]|1[012])[\\/\\-](0?[1-9]|[12][0-9]|3[01])$/", value = gsub("-", "/", patient_info_returning()$birthday), input_width = 8),
      input(session$ns("phone"), type = "tel", placeholder = "Phone", label_width = 4, input_width = 8,`data-parsley-length` = "[12,14]",
            `data-parsley-pattern` = "/^[\\d\\+\\-\\.\\(\\)\\/\\s]*$/", value = paste(
              substr(patient_info_returning()$phone, 1, 3),
              substr(patient_info_returning()$phone, 4, 6),
              substr(patient_info_returning()$phone, 7, 10)
            )),
      tags$script(paste0(
        'var cleave = new Cleave("#', session$ns("phone"),'", {
          phone: true, phoneRegionCode: "us"
        })'
      )),
      div(class ="controls form-group", style = "text-align: right;", 
          tags$label(class = "checkbox col-sm-4", `for` = session$ns("textDeal"), 
                     "Text Deals"),
          tags$input(type = "checkbox", id = session$ns("textDeal"), value = session$ns("textDeal"), name = session$ns("textDeal"),
                     class="col-sm-7"
          )
      )),
      div(class = "col-xs-6 col-sm-6 col-md-6 col-lg-6",
      input(session$ns("address"), placeholder = "Address", label_width = 4, value = patient_info_returning()$address),
      input(session$ns("city"), placeholder = "City", label_width = 4, value = patient_info_returning()$city),
      input(session$ns("state"), placeholder = "State", label_width = 4, maxlength = 2,
            `data-parsley-length` = "[2,2]", value = patient_info_returning()$state),
      input(session$ns("zip"), type = "tel", placeholder = "ZIP", label = "ZIP", label_width = 4,`data-pasley-type` = "integer",
            maxlength = 5,
            `data-parsley-length` = '[5,5]',
            onkeypress = "return event.charCode >= 48 && event.charCode <= 57", value = patient_info_returning()$zip),
      input(session$ns("email"), placeholder = "Email", label_width = 4, `data-parsley-type` = "email", value = patient_info_returning()$email),
      div(class ="controls form-group",style = "text-align: right;", 
          tags$label(class = "checkbox col-sm-4", `for` = session$ns("emailDeal"), 
                     "Email Deals"
          ),
          tags$input(type = "checkbox", id = session$ns("emailDeal"), value = session$ns("emailDeal"), name = session$ns("emailDeal"),
                     class = "col-sm-7")
      ),
      if (patient_info_returning()$emailDeal == 1) {tags$script(paste0('$("#',session$ns("emailDeal"),'").prop("checked",true)'))},
      if (patient_info_returning()$textDeal == 1) {tags$script(paste0('$("#',session$ns("textDeal"),'").prop("checked",true)'))},
      tags$script(paste0(
        'var cleaveDOB = new Cleave("#',session$ns("birthday"),'", {
          date: true, datePattern: ["Y","m","d"]
        })'
      )))
    ),footer = parsleyr::submit_form(session$ns("submit_info_edit"), "Submit",class = "btn btn-info add-queue-btn action-button",
  ...
  observeEvent(input$edit_medical_info, {
    showModal(modalDialog(
      ...
      input(session$ns("expirationDate"), placeholder = "Expiration Date", `data-parsley-pattern` = "/^\\d{4}[\\/\\-](0?[1-9]|1[012])[\\/\\-](0?[1-9]|[12][0-9]|3[01])$/",
            value = gsub("-","/",patient_info_returning()$expirationDate), label_width = 4),
      tags$script(paste0(
        'var cleaveExp = new Cleave("#',session$ns("expirationDate"),'", {
          date: true, datePattern: ["Y","m","d"]
        })'
      )),
      input(session$ns("recId"), type = "tel", placeholder = "Medicard Card #", value = patient_info_returning()$recId, label_width = 4),
      input(session$ns("medicalCondition"), placeholder = "Condition", required = FALSE, value = patient_info_returning()$medicalCondition, label_width = 4)
    ),footer = parsleyr::submit_form(session$ns("submit_medical_edit"), "Submit",class = "btn btn-info add-queue-btn action-button",
  ...
  observeEvent(input$edit_preferences_info, {
    showModal(modalDialog(
      ...
      tags$form(id = session$ns("preference_form"), class = "preference-edit", 
                ...
                tags$input(id = session$ns("indica"), type = "checkbox", value = session$ns("indica"), name = session$ns("indica"))),
                add_check("indica", patient_info_returning()),
                div(
                tags$label(`for` = session$ns("sativa"), "Sativa"),
                tags$input(id = session$ns("sativa"), type = "checkbox", value = session$ns("sativa"), name = session$ns("sativa"))),
                add_check("sativa", patient_info_returning()),
                div(
                tags$label(`for` = session$ns("hybrid"), "Hybrid"),
                tags$input(id = session$ns("hybrid"), type = "checkbox", value = session$ns("hybrid"), name = session$ns("hybrid"))),
                add_check("hybrid", patient_info_returning()),
              h3("Product Type"),
      div(
      tags$label(`for` = session$ns("flower"), "Flower"),
      tags$input(id = session$ns("flower"), type = "checkbox", value = session$ns("flower"), name = session$ns("flower"))),
      add_check("flower", patient_info_returning()),
      div(
      tags$label(`for` = session$ns("concentrate"), "Concentrate"),
      tags$input(id = session$ns("concentrate"), type = "checkbox", value = session$ns("concentrate"), name = session$ns("concentrate"))),
      add_check("concentrate", patient_info_returning()),
      div(
      tags$label(`for` = session$ns("edible"), "Edible"),
      tags$input(id = session$ns("edible"), type = "checkbox", value = session$ns("edible"), name = session$ns("edible"))),
      add_check("edible", patient_info_returning()),
      div(
      tags$label(`for` = session$ns("other"), "Other"),
      tags$input(id = session$ns("other"), type = "checkbox", value = session$ns("other"), name = session$ns("other")))),
      add_check("other", patient_info_returning()),
      footer = parsleyr::submit_form(session$ns("submit_preference_edit"), "Submit",class = "btn btn-info add-queue-btn action-button",
  ...
  observeEvent(input$edit_images, {
    showModal(modalDialog(easyClose=TRUE,tags$script("
                          ...
                          div(class = "form-group",
                              tags$label(`for` = session$ns("photoIdPath"), class = "control-label control-label-left col-sm-3", "Photo ID", span(class ="req","*")),
                              div(class = "col-sm-9",
                                  shiny::fileInput(session$ns("photoIdPath"),NULL))
                          ),
                          div(class = "form-group",
                              tags$label(`for` = session$ns("photoIdPath"), class = "control-label control-label-left col-sm-3", "Medical Card", span(class ="req","*")),
                              div(class = "col-sm-9",
                                  shiny::fileInput(session$ns("medicalPath"), NULL)
                              )
                          )
    ), footer = tags$button(id = session$ns("submit_images"), "Submit", class = "btn btn-info add-queue-btn action-button")
                          ))
  })
  ...
  output$name <- renderUI({
    if (isTruthy(patientId())) {
      h1(paste(patient_info_returning()$firstName, patient_info_returning()$lastName)) 
    } else {
  ...
  output$info <- DT::renderDataTable(
    patient_info_returning() %>% 
      mutate_(emailDeal = ~if_else(emailDeal == 1,"YES","NO"),
      ...
      t() %>% as.data.frame(stringsAsFactors=FALSE) %>% tidyr::replace_na(list(`V1`="N/A")), options = list(dom = 't', columnDefs=list(list(
        targets = 0, render= JS(
        ...
        ))), rownames = TRUE,class = "table dt-row"
        )
  ...
  output$preference <- DT::renderDataTable({
    ...
    data.frame(check.names = FALSE,
      ...
      Product = paste0(
        c("Flower", "Concentrate", "Edible", "Other")[which(c(
          info$flower == 1,
          info$concentrate ==
            1,
          info$edible ==
      ...
      `Referred By` = info$recommender
    ) %>% t() %>% as.data.frame(stringsAsFactors=FALSE) %>% tidyr::replace_na(list(`V1`="N/A"))
  ...
  output$recommendation <- DT::renderDataTable({
    patient_info_returning() %>%
      select_(`Expiration Date` = ~ expirationDate, Physician = ~ physician, `Medical Card ID #` = ~ recId,  `Medical Condtion` = ~medicalCondition) %>%
      t() %>% as.data.frame(stringsAsFactors=FALSE) %>% tidyr::replace_na(list(`V1`="N/A"))
  }, options = list(dom = 't', columnDefs=list(list(
    targets = 0, render= JS(
      "function(data, type, row, meta) {
  ...
  output$id_image_out <- renderUI({
    if (isTruthy(patientId()) && isTruthy(patient_info_returning()$photoPath)) {
    tags$img(
      src = paste0(
      ...
      tags$img(
        src = "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ac/No_image_available.svg/1000px-No_image_available.svg.png",
        height = "100%",
        width = "100%"
      )
    }
  ...
  output$recommendation_image_out <- renderUI({
    if (isTruthy(patientId()) && isTruthy(patient_info_returning()$medicalPath)) {
    tags$img(
      src = paste0(
        "https://s3-us-west-2.amazonaws.com/",
        bucket,
        ...
        div(class = "col-xs-6 col-sm-6 col-md-6 col-lg-6",
            ...
            box(class = "images",
                ...
                div(class = "col-xs-12 col-sm-12 col-md-12 col-lg-12",
                    ...
                    div(class = "col-xs-6 col-sm-6 col-md-6 col-lg-6",
                        h4("Medical Card"),
                        shiny::imageOutput(ns("new_medical_image"), inline = T, height = "auto")
                    )
                )
            )),
        div(class = "col-xs-6 col-sm-6 col-md-6 col-lg-6",
            ...
            div(class = "row",
              ...
              input(ns("recId"), "tel", placeholder = "Medical Card #", `data-parsley-type` = "integer"),
              tags$script(paste0(
                "var expDate=new Cleave('#", ns("date"),"', {
                date: true, datePattern: ['Y', 'm', 'd']
                })"
              )
            )))
...
newPatient <- function(input, output, session, pool, patientId, bucket, trigger_new, trigger_returning, proxy, parent_session, reload_patient) {
  ...
  observeEvent(input$edit_basic_info, {
    showModal(modalDialog(size= "l",
                          ...
                          tags$form(id = session$ns("basic_info_form"),
                                    div(class = "col-xs-6 col-sm-6 col-md-6 col-lg-6",
                                        input(session$ns("name"), placeholder = "First", label_width = 4, value = patient_info_new()$firstName, input_width = 8),
                                        input(session$ns("name2"), placeholder = "Last", label_width = 4, value = patient_info_new()$lastName, input_width = 8),
                                        input(session$ns("californiaID"), type = "tel", placeholder = "California ID", label = "CA ID", label_width = 4, maxlength = 8,
                                              `data-parsley-californiaid` = I(""), value = patient_info_new()$californiaID, input_width = 8),
                                        input(session$ns("birthday"), placeholder = "DOB", label = "DOB", label_width = 4, `data-parsley-year` = I(""),
                                              `data-parsley-pattern` = "/^\\d{4}[\\/\\-](0?[1-9]|1[012])[\\/\\-](0?[1-9]|[12][0-9]|3[01])$/", value = gsub("-", "/", patient_info_new()$birthday), input_width = 8)),
      div(class = "col-xs-6 col-sm-6 col-md-6 col-lg-6",
          input(session$ns("address"), placeholder = "Address", label_width = 4, value = patient_info_new()$address),
          input(session$ns("city"), placeholder = "City", label_width = 4, value = patient_info_new()$city),
          input(session$ns("state"), placeholder = "State", label_width = 4, maxlength = 2,
                `data-parsley-length` = "[2,2]", value = patient_info_new()$state),
          input(session$ns("zip"), type = "tel", placeholder = "ZIP", label = "ZIP", label_width = 4,`data-pasley-type` = "integer",
                maxlength = 5,
                `data-parsley-length` = '[5,5]',
                onkeypress = "return event.charCode >= 48 && event.charCode <= 57", value = patient_info_new()$zip))
                                    ),footer = parsleyr::submit_form(session$ns("submit_info_edit"), "Submit",class = "btn btn-info add-queue-btn action-button",
                                                                     formId = session$ns("basic_info_form"))))
  ...
  output$name <- renderUI({
    if (isTruthy(patientId())) {
      h1(paste(patient_info_new()$firstName,patient_info_new()$lastName))  
    } else {
  ...
  list(targets = 1, className = "dt-left"))), colnames = "",class = "table dt-row",
    ...
    tagList(
      div(class = "file-input",
          div(class = "form-group",
              tags$label(`for` = session$ns("photoIdPath"), class = "control-label control-label-left col-sm-3", "Photo ID", span(class ="req","*")),
              div(class = "col-sm-7",
              shiny::fileInput(session$ns("photoIdPath"),NULL, width ="100%"))
          ),
          div(class = "form-group",
              tags$label(`for` = session$ns("photoIdPath"), class = "control-label control-label-left col-sm-3", "Medical Card", span(class ="req","*")),
              div(class = "col-sm-7",
                  shiny::fileInput(session$ns("medicalPath"), NULL, width = "100%")
                  )
          ),
          tags$script(
            paste0(
              "$('#",session$ns("photoIdPath"),"').closest(\".input-group\").children(\"input\").prop(\"required\", true);
       //   $('#",session$ns("photoIdPath"),"').attr('','');\n",
              "$('#",session$ns("medicalPath"),"').closest(\".input-group\").children(\"input\").prop(\"required\", true);
        //  $('#",session$ns("medicalPath"),"').attr('','');"
            )
          )
    ))  
  })
  ...
  output$new_id_image <- renderImage({
    trigger_files()
    if (isTruthy(patientId()) && isTruthy(input$photoIdPath)) {
    list(
  ...
  output$new_medical_image <- renderImage({
    trigger_files()
    if (isTruthy(patientId()) && isTruthy(input$medicalPath)) {
    list(
...
queue <- function(input, output, session, pool, proxy, trigger, reload, parent_session) {
  # queue
  ...
  observeEvent(queue_store(), {
    ...
    if (length(obsList) < n) {
      obsList <<- c(obsList, lapply(seq_len(n-length(obsList)), function(i) {
  ...
  }, rownames = TRUE, escape = FALSE, width = "100%",options = list(dom = 't',#autoWidth = TRUE,
  ...
  }, rownames = TRUE, escape = FALSE,options = list(dom = 't',
                                                    preDrawCallback = JS('function() {
                                                             Shiny.unbindAll(this.api().table().node());}'),
                                                    drawCallback = JS('function() {
                                                     Shiny.bindAll(this.api().table().node());
...
input <-
  function(id,
    ...
    div(class = "form-group",
        tags$label(class = paste0("control-label control-label-left col-sm-", label_width),
                   `for` = id,
                   label, if (required) {
                     span(class = "req", "*")
                   }),
        div(class = paste0("controls col-sm-", input_width),
            tags$input(
              id = id,
              type = type,
              name = name,
...
add_check <- function(box, patient_info_returning, session = shiny::getDefaultReactiveDomain()) {
  if (patient_info_returning[[box]] == 1) {
    tags$script(
      paste0(
        '$("#',session$ns(box),'").prop("checked",true)'
      )
    )
  }
}